/**
 * Casa Certa — Site Initializer  (v2)
 * Injeta dados dinâmicos da API no HTML existente sem alterar o visual.
 *
 * NOVIDADES v2:
 *  - applyImoveis: "Ver Mais" agora abre a página individual do imóvel (/imovel.html?id=…)
 *  - applyIcons(): injeta ícones SVG oficiais do WhatsApp e Instagram
 *  - applyFooterCredit(): insere "Desenvolvido por Algo Urgente" no rodapé
 */
...
const CC = {
  ...
  async init() {
    try {
      ...
      try { CC.applySEO(); }           catch (e) { console.warn('[CC] applySEO:', e); }
    } catch (e) {
  ...
  applyImoveis(list) {
    ...
    grid.innerHTML = list.map((im, i) => {
      ...
      // ✅ ALTERADO: "Ver Mais" agora vai para a página individual do imóvel
      const paginaImovel = `/imovel.html?id=${im.id}`;

      // WhatsApp continua disponível no botão dedicado
      const wpp = `https://wa.me/${window.WPP_DIOGO || '5588981545786'}?text=${encodeURIComponent(`Olá! Tenho interesse no imóvel: ${im.titulo}`)}`;
  ...
  applyMarquee(items) {
    ...
    const all = [...list, ...list]; // duplica para scroll infinito
  ...
  /* ── ÍCONES SOCIAIS ──────────────────────────────────────────────────────
   * Injeta os ícones oficiais do WhatsApp e Instagram nos elementos do HTML
   * que possuem os atributos data-wpp ou data-cc="instagram_url".
   * Seguro para chamar múltiplas vezes (verifica se o ícone já foi injetado).
   * ──────────────────────────────────────────────────────────────────────── */
  applyIcons() {
    // Injetar estilos de ícone uma única vez
    ...
    document.querySelectorAll('[data-wpp]').forEach(el => {
      if (el.querySelector('.cc-social-icon')) return; // já injetado
    ...
    // ── Botões genéricos com texto WhatsApp / Instagram ───────────────────
    // (para casos onde o HTML usa texto simples sem data-attrs)
    document.querySelectorAll('a[href*="wa.me"], a[href*="whatsapp.com"]').forEach(el => {
      if (el.querySelector('.cc-social-icon')) return;
      if (el.dataset.wpp) return; // já tratado acima
      el.classList.add('cc-icon-wrap');
      el.insertAdjacentHTML('afterbegin', `<span style="display:inline-flex;align-items:center;color:#25D366">${CC_ICON_WPP}</span>`);
    });
    ...
    document.querySelectorAll('a[href*="instagram.com"]').forEach(el => {
      if (el.querySelector('.cc-social-icon')) return;
      if (el.dataset.cc === 'instagram_url') return; // já tratado acima
      el.classList.add('cc-icon-wrap');
   ...
   * Insere o crédito no rodapé do site público.
   * Procura por <footer>, .footer, [data-cc-section="footer"] ou
   * o elemento com data-cc="footer_copyright".
   * ──────────────────────────────────────────────────────────────────────── */
  applyFooterCredit() {
    // Evita duplicar em caso de re-chamada
    ...
    // Tenta encontrar o melhor container para o crédito (em ordem de preferência)
    const target =
    ...
    if (target) {
      ...
      // Fallback: insere fixado ao fundo da página
      credit.style.cssText += ';padding:16px;border-top:1px solid rgba(34,197,94,.1)';
...
/* ── Auto-init ── */
/* ── Configuração Segura do Escopo Global ── */
// Garante que o objeto CC fique disponível globalmente antes do auto-init
window.CC = window.CC || CC;

/* ── Auto-init (Corrigido) ── */
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    if (window.CC && typeof window.CC.init === 'function') {
      window.CC.init();
    }
  });
} else {
...
// Dentro do seu CC.init() ou no final do site-init.js:
